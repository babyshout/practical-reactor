...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    public void transforming_sequence_2() {
        ...
        Flux<String> resultSequence = numbersFlux.map(integer -> integer > 0 ? ">" : integer == 0 ? "=" : "<");
...
//                .map(i -> (String) i); //todo: change this line only
    ...
    public void sequence_sum() {
        ...
        Mono<Integer> sum =
//                numerical_service().reduce((integer, integer2) -> integer + integer2).log();
                numerical_service().reduce(0, (integer, integer2) -> integer + integer2).log();
    ...
    public void sum_each_successive() {
        Flux<Integer> sumEach = numerical_service()
                //todo: do your changes here
//                .reduceWith(() -> numerical_service().blockFirst(), (integer, integer2) -> integer + integer2)
//                .flatMap(integer -> ).join()
                .scan(Integer::sum)
                ;
